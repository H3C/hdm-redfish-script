...
class AddLdapUser(BaseModule):
    ...
    def run(self, args):
        ...
        try:
            ldap_config_url = "/api/settings/ldap-settings"
            resp = client.send_request("GET", ldap_config_url)
            if not isinstance(resp, dict) or Constant.SUCCESS_0 == resp.get(
                    "cc", None):
                if 0 == resp.get("enable", None):
                    self.err_list.append(
                        "Failure: ldap disable, can not add ldap user")
                    raise FailException(*self.err_list)
            else:
                self.err_list.append("Failure: failed to add ldap user")
                raise FailException(*self.err_list)
            payload = self._construct_param(args)
            add_url = "/api/settings/ldap-users"
            ...
            if (isinstance(resp, dict) and Constant.SUCCESS_0 == resp.get(
                ...
                self.suc_list.append("Success: add ldap user successfully")
            else:
                self.err_list.append("Failure: failed to add ldap user")
                raise FailException(*self.err_list)
        finally:
    ...
    def _construct_param(self, args):
        ...
        payload["role_group_withoem_privilege"] = 15
        return payload
